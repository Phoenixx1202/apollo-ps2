...
static void downloadSave(const save_entry_t* entry, const char* file, int dst)
{
	...
	{
		show_message("Erro! Pasta de exportação indisponível:\n%s", path);
		return;
	}

	if (!http_download(entry->path, file, APOLLO_LOCAL_CACHE "tmpsave.zip", 1))
	{
		show_message("Erro ao baixar jogo salvo de:\n%s%s", entry->path, file);
		return;
	}

	if (extract_zip(APOLLO_LOCAL_CACHE "tmpsave.zip", path))
		show_message("Jogo salvo baixado com sucesso em:\n%s", path);
	else
		show_message("Erro ao extrair jogo salvo!!");
...
static void zipSave(const save_entry_t* entry, int dst)
{
	...
	init_loading_screen("Exportando Jogo Salvo...");
...
//	save_xml_owner(export_file);
	...
	stop_loading_screen();
	show_message("Arquivo Zip salvo com sucesso em::\n%s%08d.zip", exp_path, fid);
}
...
static void copySave(const save_entry_t* save, int dev)
{
	...
	{
		show_message("Operação de cópia cancelada! Mesma origem e destino.");
		return;
	}
	...
	init_loading_screen("Copiando arquivos...");

	asprintf(&copy_path, "%s%s/", exp_path, save->dir_name);

	LOG("Copiando <%s> para %s...", save->path, copy_path);
	copy_directory(save->path, save->path, copy_path);

	free(copy_path);

	stop_loading_screen();
	show_message("Arquivos copiados com sucesso para:\n%s", exp_path);
}
...
static void copySaveMC(const save_entry_t* save, int dev)
{
	...
	{
		show_message("Operação de cópia cancelada!\nMesma origem e destino");
		return;
	}

	init_loading_screen("Copiando dados salvos...");
	...
	if (ret)
		...
		show_message("Erro! Falha ao copiar a pasta de Dados Salvos:\n%s", save->path);
}
...
static void copyAllSavesMC(const save_entry_t* save, int dev, int all)
{
	...
	init_progress_bar("Copiando todos os dados salvos...");
	_set_dest_path(mc_path, dev, "");

	LOG("Copiando todos os dados salvos de '%s' para mc%d:/...", save->path, dev);
	...
	show_message("%d/%d Dados Salvos copiados para o Memory Card %d", done, done+err_count, dev+1);
}
...
static void exportAllSavesVMC(const save_entry_t* save, int dev, int all)
{
	...
	init_progress_bar("Exportando todos os dados salvos VMC...");
	_set_dest_path(outPath, dev, PS3_SAVES_PATH_USB);
	mkdirs(outPath);

	LOG("Exportando todos os dados salvos de '%s' para mc%d:/...", save->path, dev);
	for (node = list_head(list); (item = list_get(node)); node = list_next(node))
	{
		update_progress_bar(progress++, list_count(list), item->name);
		if (!all && !(item->flags & SAVE_FLAG_SELECTED))
	...
	show_message("%d/%d Dados Salvos exportados para\n%s", done, done+err_count, outPath);
}
...
//		ret = extract_7zip(file_path, exp_path);
	...
	if (ret)
		show_message("Todos os arquivos extraídos para:\n%s", exp_path);
	else
		show_message("Erro: %s não pôde ser extraído", file_path);
}
...
static void importSave(const save_entry_t* save, const char* mc_path)
{
	...
	LOG("Importando Dados Salvos '%s'... {%s}", save->path, mc_path);
	init_progress_bar("Importando Dados Salvos...");
	...
	if (ret)
		show_message("Dado Salvo importado com sucesso para %s", mc_path);
	else
		show_message("Erro: Não foi possível importar o dado salvo");
}
...
static void copyAllSavesUSB(const save_entry_t* save, int dev, int all)
{
	...
	{
		show_message("Erro! Pasta não está disponível:\n%s", dst_path);
		return;
	}

	init_progress_bar("Copiando todos os dados salvos...");

	LOG("Copiando todos os dados salvos para '%s'...", dst_path);
	for (node = list_head(list); (item = list_get(node)); node = list_next(node))
	{
		update_progress_bar(progress++, list_count(list), item->name);
		if (!all && !(item->flags & SAVE_FLAG_SELECTED))
		...
		LOG("Copiando <%s> para %s...", item->path, copy_path);
	...
	end_progress_bar();
	show_message("%d/%d Dados Salvos copiados para:\n%s", done, done+err_count, dst_path);
}
...
static void* ps2_host_callback(int id, int* size)
{
	...
	case APOLLO_HOST_LAN_ADDR:
		if (1)//(sceWlanGetEtherAddr(host_buf) < 0)
			LOG("Erro ao obter Endereço Ethernet da LAN");
...
static int apply_cheat_patches(const save_entry_t* entry)
{
	...
	init_loading_screen("Aplicando alterações...");
	...
	for (node = list_head(entry->codes); (code = list_get(node)); node = list_next(node))
	{
		...
		else
		{
			...
			LOG("Descriptografando arquivo '%s'", tmpfile);
...
if(0)//				if (get_psp_save_key(entry, key) && psp_DecryptSavedata(entry->path, tmpfile, key))
				{
					LOG("Arquivo PSP Descriptografado '%s'", filename);
					list_append(decrypted_files, strdup(filename));
				}
				else
				{
					LOG("Erro: Falha ao descriptografar (%s)", filename);
					ret = 0;
					continue;
				}
			}
		...
		{
			LOG("Erro: Não foi possível aplicar (%s)", code->name);
			ret = 0;
		}
	...
	{
		LOG("Criptografando '%s'...", filename);
if(0)//		if (!get_psp_save_key(entry, key) || !psp_EncryptSavedata(entry->path, filename, key))
		{
			LOG("Erro: Não foi possível criptografar (%s)", filename);
			ret = 0;
		}
...
{
    LOG("Reassinando dado salvo '%s'...", entry->name);

    LOG("Aplicando trapaças em '%s'...", entry->name);
    if (!apply_cheat_patches(entry))
        show_message("Erro! Os códigos de trapaça não puderam ser aplicados");

    show_message("Jogo salvo %s modificado com sucesso!", entry->title_id);
}
...
static int deleteSave(const save_entry_t* save)
{
	...
	if (!show_dialog(DIALOG_TYPE_YESNO, "Você deseja excluir %s?", save->dir_name))
		return 0;
	...
	if (ret)
		show_message("Dado Salvo excluído com sucesso:\n%s", save->dir_name);
	else
		show_message("Erro! Não foi possível excluir o jogo salvo:\n%s", save->dir_name);
...
static void exportAllSaves(const save_entry_t* save, int dev, int all)
{
	...
	init_progress_bar("Exportando todos os dados salvos...");
	_set_dest_path(outPath, dev, (save->flags & SAVE_FLAG_PS1) ? PS1_SAVES_PATH_USB : PS2_SAVES_PATH_USB);
	mkdirs(outPath);

	LOG("Exportando todos os dados salvos de '%s' para %s...", save->path, outPath);
	for (node = list_head(list); (item = list_get(node)); node = list_next(node))
	{
		update_progress_bar(progress++, list_count(list), item->name);
		if (!(all || item->flags & SAVE_FLAG_SELECTED))
	...
	*strrchr(outPath, '/') = 0;
	show_message("%d/%d Jogos salvos exportados para:\n%s", done, done+err_count, outPath);
}
...
static void export_ps2save(const save_entry_t* save, int type, int dst_id)
{
	...
	init_progress_bar("Exportando Dado Salvo...");
	...
	if (ret)
		show_message("Jogo salvo exportado com sucesso para:\n%s", outPath);
	else
		show_message("Erro ao exportar jogo salvo:\n%s", save->path);
}
...
static void export_ps1vmc(const char* vm1_file, int dst, int vmp)
{
	...
	if (saveMemoryCard(dstfile, vmp ? PS1CARD_VMP : PS1CARD_RAW, 0))
		show_message("Memory card exportado com sucesso para:\n%s", dstfile);
	else
		show_message("Erro! Falha ao exportar o memory card do PS1");
}
...
static void import_save2vmc(const char* src, int type)
{
	...
	if (ret)
		show_message("Importado com sucesso para VMC:\n%s", src);
	else
		show_message("Erro ao importar jogo salvo:\n%s", src);
}

static void copyVmcSave(const save_entry_t* save, const char* mc_path)
{
	init_progress_bar("Copiando dados salvos...");
...
static void exportSaveFile(const save_entry_t* entry, const char* filename)
{
	...
	LOG("Descriptografar '%s%s' para '%s'...", entry->path, filename, path);
...
static void importSaveFile(const save_entry_t* entry, const char* filename)
{
	...
	{
		show_message("Erro! Arquivo de jogo salvo não encontrado:\n%s", path);
		return;
	}

	snprintf(path, sizeof(path), APOLLO_USER_PATH "%s/", entry->title_id);
	LOG("Criptografar '%s%s' para '%s'...", path, filename, entry->path);
...
static void downloadLink(const char* path)
{
	...
	if (!osk_dialog_get_text("URL de Download", url, sizeof(url)))
		return;

	snprintf(out_path, sizeof(out_path), "%s%s", path, strrchr(url, '/')+1);

	if (http_download(url, NULL, out_path, 1))
		show_message("Arquivo baixado com sucesso para:\n%s", out_path);
	else
		show_message("Erro! O arquivo não pôde ser baixado");
}
	...
	switch (codecmd[0])
	{
		...
		case CMD_IMP_VMC1SAVE:
			if (openSingleSave(code->file, (int*) host_buf))
				show_message("Jogo salvo importado com sucesso:\n%s", code->file);
			else
				show_message("Erro! Não foi possível importar jogo salvo:\n%s", code->file);
